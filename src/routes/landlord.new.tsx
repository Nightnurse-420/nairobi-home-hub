...
export const Route = createFileRoute("/landlord/new")({
  head: () => ({ meta: [{ title: "New listing · NyumbaSearch" }] }),
...
function NewListingPage() {
  ...
  useEffect(() => {
    ...
    else if (!isLandlord) navigate({ to: "/profile" });
  ...
  return (
    <div className="px-5 pb-10 pt-6">
      <Link to="/landlord" className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-card shadow-card">
        <ArrowLeft className="h-4 w-4" />
      </Link>
      <h1 className="mt-5 font-display text-2xl font-extrabold">List a property</h1>
      <p className="text-sm text-muted-foreground">Reach verified tenants directly — no agents.</p>

      <form onSubmit={submit} className="mt-6 space-y-5">
        <Field label="Listing title">
          <input required value={title} onChange={e => setTitle(e.target.value)} placeholder="Sunlit 2BR with balcony" className={inputCls} />
        </Field>

        <Field label="Description">
          <textarea required value={description} onChange={e => setDescription(e.target.value)} rows={4} placeholder="What makes this home special?" className={cn(inputCls, "resize-none")} />
        </Field>
        ...
        <Field label="WhatsApp number" sub="Tenants will message you here">
