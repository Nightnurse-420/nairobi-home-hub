...
import { Bell, ShieldCheck, HelpCircle, Settings, LogIn, LogOut, Sparkles, Building2, KeyRound, Heart, Loader2 } from "lucide-react";
...
function ProfilePage() {
  const { user, profile, isLoading, isLandlord, isOwner, isAdmin, primaryRole, signOut } = useAuth();

  if (isLoading) {
    return <div className="flex min-h-[60vh] items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>;
  }

  const roleLabel = primaryRole === "admin" ? "Admin" : primaryRole === "landlord" ? "Landlord" : primaryRole === "apartment_owner" ? "Apartment Owner" : "Tenant";
    ...
    <div className="px-5 pb-4 pt-6">
      ...
      <section className="mt-6">
        ...
        <div className="mt-2 divide-y divide-border overflow-hidden rounded-3xl bg-card shadow-card">
          {user && isAdmin && (
            <Row to="/admin/role-requests" icon={ShieldCheck} label="Role approvals" sub="Review landlord & owner requests" />
          )}
          {user && isLandlord && (
            <Row to="/landlord" icon={Building2} label="Landlord dashboard" sub="Manage buildings & inquiries" />
          )}
          {user && isOwner && (
            <Row to="/owner" icon={KeyRound} label="Apartment owner dashboard" sub="Manage your unit & finances" />
          )}
          {user && !isLandlord && !isOwner && (
            <Row to="/onboarding/role" icon={Building2} label="List your property" sub="Apply to host as landlord or owner" />
          )}
